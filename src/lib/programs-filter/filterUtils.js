// lib/programs-filter/filterUtils.js
//
...
/* ----------------------------------------------------------------
   FACET SLUG MAPS
   For any checkbox spec flagged `urlEncode: "slug"` (e.g. School), this
   builds a reversible mapping between each raw value and a clean slug
   (from the full, unfiltered dataset — same "compute once" pattern as
   buildStableFacetOrder). The app's internal filter state always keeps
   the raw value (so it matches program.school exactly); only the URL
   representation is swapped to the slug via this map.
   Collisions (two different raw values slugifying to the same string)
   are disambiguated with a numeric suffix so the mapping always stays
   reversible, even on unusual data.
   ---------------------------------------------------------------- */
export function buildFacetSlugMaps(programs, specs) {
  const maps = {};
  specs.forEach((spec) => {
    if (spec.type === "range" || spec.urlEncode !== "slug") return;

    const rawValues = [...tallyCounts(programs, spec.getValue).keys()];
    const slugToRaw = new Map();
    const rawToSlug = new Map();

    rawValues.forEach((raw) => {
      const base = slugify(raw) || "value";
      let candidate = base;
      let n = 2;
      while (slugToRaw.has(candidate)) {
        candidate = `${base}-${n++}`;
      }
      slugToRaw.set(candidate, raw);
      rawToSlug.set(raw, candidate);
    });

    maps[spec.key] = { slugToRaw, rawToSlug };
  });
  return maps;
}
...
export function buildFilterSpecs(programs) {
  const specs = [
    ...
    },
    {
      ...
      // Written to the URL as a slug (school-of-engineering...) instead
      // of the raw display name — see buildFacetSlugMaps below. Add
      // this same flag to any other checkbox spec you want slugified.
      urlEncode: "slug",
    },
  ];
...
/* ----------------------------------------------------------------
   10. URL <-> FILTER STATE
   Encoding scheme (kept human-readable/shareable):
     - checkbox facet "level" with 2 selections -> repeated params:
         ?level=Bachelor%27s&level=Master%27s
     - checkbox facet flagged `urlEncode: "slug"` (e.g. School) ->
         repeated params using slugs instead of raw display text:
         ?school=school-of-engineering-and-information-technology
     - range facet "fee" -> two params: ?feeMin=10000&feeMax=50000
     - free-text search -> ?q=pharmacy
   Works with both the browser's native URLSearchParams and Next.js's
   ReadonlyURLSearchParams (useSearchParams()), since both implement
   .get / .getAll / .toString.
   `slugMaps` (from buildFacetSlugMaps) is optional — omit it and
   slug-flagged facets simply pass their values through unencoded.
   ---------------------------------------------------------------- */
export function parseFiltersFromSearchParams(
  ...
  specs.forEach((spec) => {
    if (spec.type === "range") {
      const minRaw = searchParams.get(`${spec.key}Min`);
      const maxRaw = searchParams.get(`${spec.key}Max`);
    ...
    const rawParamValues = searchParams.getAll(spec.key).filter(Boolean);
    if (!rawParamValues.length) return;

    const map = spec.urlEncode === "slug" ? slugMaps[spec.key] : null;
    const values = map
      ? rawParamValues.map((v) => map.slugToRaw.get(v)).filter(Boolean) // unknown/stale slugs are dropped
      : rawParamValues;

    if (values.length) filters[spec.key] = values;
  });
...
// Builds a URLSearchParams reflecting the given filter state. Starts
// from `baseSearchParams` (if provided) so any params this app doesn't
// manage are left untouched — only the keys this module owns are
// cleared and rewritten. `slugMaps` is optional — omit it and
// slug-flagged facets fall back to writing their raw value.
export function buildSearchParamsFromFilters(
  ...
  specs.forEach((spec) => {
    if (spec.type === "range") {
      params.delete(`${spec.key}Min`);
      params.delete(`${spec.key}Max`);
    } else {
  ...
  specs.forEach((spec) => {
    if (spec.type === "range") {
      const range = filters[spec.key];
      if (!range) return;
      if (range.min !== null && range.min !== undefined) {
        params.set(`${spec.key}Min`, String(range.min));
      }
      if (range.max !== null && range.max !== undefined) {
        params.set(`${spec.key}Max`, String(range.max));
      }
      return;
    }
    ...
    const map = spec.urlEncode === "slug" ? slugMaps[spec.key] : null;
    values.forEach((v) => {
      const encoded = map ? map.rawToSlug.get(v) || slugify(v) : v;
      params.append(spec.key, encoded);
    });
  });
